...
import urllib.request, urllib.error, urllib.parse, urllib.request, urllib.parse, urllib.error, sys, argparse
logo = '''
'''
...
def curllib(req, params=None,postdata=None):
    ...
    try:
        ...
        req = urllib.request.urlopen(req, timeout = 30).read()
...
for x in range( 0, usern ):
    ...
    try:
        tmp = curllib(site, '', urllib.parse.urlencode({"author":(x+1)}) )#vsend the request
        if tmp == False:
            pass
